...
int main(int argc, char *argv[]) {
  ...
  std::cout << "[ArchivumDocs] Extraction complete. Found "
            << extracted_nodes.size() << " structural boundaries.\n\n";

  for (const auto &node : extracted_nodes) {
    graph.register_node(node);

    std::string type_str =
        (node.type == archivum::NodeType::FUNCTION) ? "FUNCTION"
        : (node.type == archivum::NodeType::CLASS)  ? "CLASS"
                                                    : "STRUCT";

    std::cout << "  -> [" << type_str << "] " << node.name << " \t(Lines "
              << node.start_line << " to " << node.end_line << ")"
              << " \tID: " << node.id << "\n";
  }
